...
public:
    ...
    bool init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            jnt_state_iface_.registerHandle(
                hardware_interface::JointStateHandle(joint_names[i], &pos_[i], &vel_[i], &eff_[i]));
            jnt_vel_iface_.registerHandle(
                hardware_interface::JointHandle(jnt_state_iface_.getHandle(joint_names[i]), &cmd_[i]));
        }
        ...
        if (::connect(udp_fd_, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr)) < 0)
        {
            ROS_FATAL("mote_node: connect() to %s:%d failed: %s",
                      robot_ip.c_str(), UDP_PORT, std::strerror(errno));
            return false;
        }

        // Advertise sensor topics (joint_states and odom published by diff_drive_controller)
    ...
    void read(const ros::Time& time, const ros::Duration& /*period*/) override
    {
        ...
        while (true)
        {
            int n;
            {
                ...
                n = mote_link_poll_receive(link_, buf.data(), static_cast<int>(buf.size()));
            }
            if (n == 0) break;
            if (n < 0)
            {
                ROS_WARN_THROTTLE(5.0, "mote_node: mote_link_poll_receive error (buffer too small?)");
                break;
            }
            try
            {
    ...
    // Called after cm.update(). Sends the velocity commands set by the controller.
    void write(const ros::Time& /*time*/, const ros::Duration& /*period*/) override
    {
        json cmd = {
            {"DriveBaseCommand", {
                {"left_velocity_rad",  cmd_[0]},
                {"right_velocity_rad", cmd_[1]}
            }}
        };
...
private:
    ...
    double cmd_[2] = {0.0, 0.0};  // written by diff_drive_controller via VelocityJointInterface
    ...
    void flush_transmit()
    {
        ...
        int n;
        while ((n = mote_link_poll_transmit(link_, pkt.data(), static_cast<int>(pkt.size()))) > 0)
        {
            if (::send(udp_fd_, pkt.data(), static_cast<std::size_t>(n), 0) < 0)
                ROS_WARN_THROTTLE(5.0, "mote_node: UDP send() failed: %s", std::strerror(errno));
        }
        if (n < 0)
            ROS_WARN_THROTTLE(5.0, "mote_node: mote_link_poll_transmit: buffer too small");
    }
    ...
    void recv_thread_fn()
    {
        ...
        while (running_.load())
        {
            ...
            if (r < 0)
            {
                ...
                ROS_WARN_THROTTLE(5.0, "mote_node: select() error: %s", std::strerror(errno));
                continue;
            }
            if (r == 0) continue;  // timeout — check running_ and loop
    ...
    {
        // Note: field name "postition_rad" is a typo in the mote-api schema (double-t).
        // It must match the wire format exactly.
    ...
    // Rasterize a LiDAR scan onto a fixed 0.5° angle grid and publish as LaserScan.
